...
console.log(message.content);`,
  ...
  const sectionLabels = zh
    ? ["接入前准备", "控制台功能", "令牌与模型", "第一次调用", "路由与故障兜底", "额度与计费", "图片、音频、视频", "安全使用", "错误排查"]
    : ["Before you start", "Console features", "Tokens and models", "First request", "Routing and failover", "Balance and billing", "Images, audio, video", "Security", "Troubleshooting"];
    ...
    <div className="space-y-6 pb-10">
      ...
      <div className="grid gap-6 lg:grid-cols-[220px_minmax(0,1fr)] xl:grid-cols-[240px_minmax(0,1fr)]">
        ...
        <main className="min-w-0 space-y-6">
          ...
          <section id="console-workspace" className="scroll-mt-24">
            <Card className="border-slate-200/80 shadow-sm dark:border-slate-800 dark:bg-slate-900/60">
              <CardHeader>
                <CardTitle className="flex items-center gap-2 text-lg"><BookOpen className="h-5 w-5 text-indigo-500" />{zh ? "控制台功能" : "Console features"}</CardTitle>
                <CardDescription>{zh ? "控制台页面用于管理令牌、查看用量、充值和测试接口；这些页面不会暴露上游密钥。" : "The console manages tokens, usage, billing, and interface tests without exposing upstream credentials."}</CardDescription>
              </CardHeader>
              <CardContent className="space-y-3">
                <div className="grid gap-3 sm:grid-cols-2">
                  <InfoStep number="01" icon={Network} tone="indigo" title={zh ? "总览与用量分析" : "Dashboard and analytics"} body={zh ? "总览默认统计今日 00:00 至当前时间，并提供昨天、3/7/15/30/60/90 天和自定义时间筛选；模型分布和 Token 趋势按所选范围计算。" : "The dashboard defaults to today from 00:00 to now, with yesterday, 3/7/15/30/60/90-day, and custom ranges. Model distribution and token trends follow the selected range."} />
                  <InfoStep number="02" icon={CircleDollarSign} tone="amber" title={zh ? "费用中心与订单中心" : "Billing and orders"} body={zh ? "费用中心显示余额和全局充值套餐；订单中心显示充值状态、到账额度和支付结果。充值订单必须由服务端支付回调确认，不能仅凭浏览器跳转入账。" : "Billing shows balance and global recharge packages; Orders shows status, credited balance, and payment results. Credit is confirmed by server-side payment callbacks, never by a browser redirect alone."} />
                  <InfoStep number="03" icon={Code2} tone="cyan" title={zh ? "接口调试" : "Interface debugging"} body={zh ? "文本模型调试、图片模型调试和视频模型调试仅用于当前浏览器测试，不会保存测试 API Key；请求体和 cURL 可直接复制。" : "Text, image, and video debugging runs in the current browser only and does not store the test API key. Request bodies and cURL examples can be copied."} />
                  <InfoStep number="04" icon={ShieldCheck} tone="emerald" title={zh ? "模型状态与企业认证" : "Model status and enterprise verification"} body={zh ? "模型状态取决于管理员启用的监控配置；企业认证资料只返回脱敏银行账号，审核状态可在控制台查看。" : "Model status follows administrator-enabled monitors. Enterprise verification masks bank accounts and exposes the review status in the console."} />
                </div>
                <div className="rounded-xl border border-indigo-500/20 bg-indigo-500/[0.06] p-4 text-xs leading-6 text-slate-700 dark:text-slate-300"><span className="font-bold text-indigo-700 dark:text-indigo-300">{zh ? "仪表盘接口：" : "Dashboard endpoint: "}</span><code>GET /console/v1/tenants/&#123;tenantID&#125;/dashboard</code><br />{zh ? "用量明细接口：" : "Usage endpoint: "}<code>GET /console/v1/tenants/&#123;tenantID&#125;/usage</code></div>
                <div className="grid gap-3 sm:grid-cols-2">
                  <div className="rounded-xl border border-slate-200/80 p-4 dark:border-slate-800">
                    <div className="flex items-center gap-2 text-sm font-bold text-slate-900 dark:text-white"><KeyRound className="h-4 w-4 text-indigo-500" />{zh ? "快捷登录" : "Quick sign-in"}</div>
                    <p className="mt-2 text-xs leading-5 text-slate-600 dark:text-slate-400">{zh ? "管理员启用后，登录和注册页可使用 Google、GitHub、Linux.do。OAuth 密钥仅保存在服务端。" : "When enabled by an administrator, Google, GitHub, and Linux.do appear on sign-in and registration. OAuth secrets stay on the server."}</p>
                    <code className="mt-2 block break-words text-[10px] leading-5 text-indigo-700 dark:text-indigo-300">GET /public/v1/auth/providers<br />/console/v1/auth/oauth/&#123;provider&#125;/start</code>
                  </div>
                  <div className="rounded-xl border border-slate-200/80 p-4 dark:border-slate-800">
                    <div className="flex items-center gap-2 text-sm font-bold text-slate-900 dark:text-white"><CircleDollarSign className="h-4 w-4 text-amber-500" />{zh ? "全局充值套餐" : "Global recharge packages"}</div>
                    <p className="mt-2 text-xs leading-5 text-slate-600 dark:text-slate-400">{zh ? "套餐配置与支付方式同级，所有支付方式共用。可设置充值金额、到账金额、赠送金额、活动时间和有效期；有效期为 0 表示长期有效，订阅字段为后续预留。" : "Packages are a separate system setting shared by every payment provider. Configure paid, credited, bonus amounts, campaign dates, and validity; 0 means permanent, and subscription fields are reserved for later."}</p>
                    <code className="mt-2 block break-words text-[10px] leading-5 text-amber-700 dark:text-amber-300">GET /admin/v1/settings/payment-packages<br />GET /public/v1/payments/providers</code>
                  </div>
                </div>
                <div className="flex items-start gap-2 rounded-xl border border-cyan-500/20 bg-cyan-500/[0.06] p-4 text-xs leading-5 text-slate-700 dark:text-slate-300"><ShieldCheck className="mt-0.5 h-4 w-4 shrink-0 text-cyan-600" /><span>{zh ? "模型状态由管理员监控配置控制：被动模式根据真实请求健康记录更新；主动模式保存后立即探测一次，之后按间隔探测监控范围内模型和候选渠道。探测结果参与健康判断并写入系统健康历史，但不计入客户账单或客户用量。" : "Model status follows administrator monitor settings. Passive mode uses real request health records; active mode probes once after saving and then probes the configured models and candidate channels on schedule. Probe results affect health and are kept in system history, but are excluded from customer billing and usage."}</span></div>
                <div className="flex flex-wrap gap-2"><Button type="button" variant="outline" size="sm" onClick={() => routeTo("#console/dashboard")} className="gap-2"><Network className="h-3.5 w-3.5" />{zh ? "打开控制台总览" : "Open dashboard"}<ArrowRight className="h-3.5 w-3.5" /></Button><Button type="button" variant="outline" size="sm" onClick={() => routeTo("#console/interface-debug-text")} className="gap-2"><Code2 className="h-3.5 w-3.5" />{zh ? "打开接口调试" : "Open interface debugging"}<ArrowRight className="h-3.5 w-3.5" /></Button></div>
              </CardContent>
            </Card>
          </section>
            ...
            <Card className="border-slate-200/80 shadow-sm dark:border-slate-800 dark:bg-slate-900/60">
              ...
              <CardContent className="space-y-4">
                <div className="grid gap-3 sm:grid-cols-2">
                  ...
                  <div className="rounded-xl border border-slate-200 bg-slate-50 p-4 dark:border-slate-800 dark:bg-slate-950/30"><div className="text-[11px] font-bold uppercase tracking-[0.14em] text-slate-400">Authorization</div><code className="mt-2 block break-all text-xs text-emerald-700 dark:text-emerald-300">Bearer YOUR_GATEWAY_TOKEN</code><p className="mt-2 text-[11px] leading-5 text-slate-500">{zh ? "创建成功后显示完整令牌；当前浏览器会保存新创建的密钥，刷新后仍可复制。" : "The full token is shown after creation. New secrets are kept in this browser so they remain copyable after refresh."}</p></div>
                </div>
          ...
          <section id="billing" className="scroll-mt-24">
            <Card className="border-slate-200/80 shadow-sm dark:border-slate-800 dark:bg-slate-900/60">
              ...
              <CardContent className="space-y-4">
                ...
                <p className="text-xs leading-5 text-slate-500 dark:text-slate-400">{zh ? "上游没有返回且无法验证的用量不会按 0 元放行。图片数量、请求字符数、明确的视频秒数等可验证计量会与上游 Usage 合并记录；媒体请求未提供 Idempotency-Key 时，平台会使用请求 ID 作为幂等键。" : "Unknown usage is never silently treated as zero. Verifiable quantities such as image count, request characters, and explicit video seconds are recorded alongside upstream Usage. If a media request omits Idempotency-Key, the platform derives one from its request ID."}</p>
              </CardContent>
            </Card>
          </section>
