...
const stripeMethodLabels: Record<string, TranslationKey> = {
  card: "paymentStripeCard",
  alipay: "paymentStripeAlipay",
  wechat_pay: "paymentStripeWechatPay",
};
  ...
  return (
    <Card className="border-emerald-500/20 shadow-sm dark:bg-slate-900/60">
      ...
      <CardContent className="space-y-5">
        ...
        {activeProviders.length === 0 ? <div className="rounded-xl border border-dashed border-slate-300 px-4 py-10 text-center text-sm text-slate-500 dark:border-slate-700 dark:text-slate-400">{t("rechargeUnavailable")}</div> : <form className="space-y-6 rounded-2xl border border-emerald-500/20 bg-emerald-50/40 p-4 dark:bg-emerald-500/5 sm:p-6" onSubmit={(event) => void submit(event)}>
          <div className="space-y-3"><div className="flex items-center justify-between gap-3"><Label>{t("rechargeAmount")}</Label><span className="text-xs text-slate-500">{t("rechargeCustomAmountHint")}</span></div>{packages.length > 0 ? <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">{packages.map((item) => <PackageOption key={item.id} item={item} currency={currency} rate={rate} selected={packageID === item.id} t={t} onSelect={() => { setPackageID(item.id); setAmount(item.amount); }} />)}</div> : <p className="rounded-lg border border-dashed border-slate-300 px-3 py-3 text-xs text-slate-500 dark:border-slate-700">{t("rechargeCustomOnly")}</p>}<div className="max-w-sm"><Label htmlFor="recharge-amount" className="sr-only">{t("rechargeAmount")}</Label><div className="flex h-11 items-center rounded-xl border border-slate-200 bg-white shadow-sm focus-within:border-indigo-500 focus-within:ring-2 focus-within:ring-indigo-500/10 dark:border-slate-700 dark:bg-slate-950/70"><span className="border-r border-slate-200 px-3 text-sm font-semibold text-slate-500 dark:border-slate-700">{currency}</span><Input id="recharge-amount" inputMode="decimal" min="0.01" value={amount} onChange={(event) => { setAmount(event.target.value); setPackageID(""); }} placeholder={legacyPresets[0] || t("rechargeCustomAmountPlaceholder")} required className="h-full border-0 bg-transparent shadow-none focus-visible:ring-0" /></div><p className="mt-2 text-xs text-slate-500">{t("rechargeRateHint")} {rate}</p></div></div>
          <div className="space-y-3"><Label>{t("rechargeProvider")}</Label><div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">{activeProviders.map((item) => { const active = selectedProvider?.provider === item.provider; const methods = item.payment_method_types || []; return <button key={item.provider} type="button" onClick={() => selectProvider(item.provider)} className={cn("flex min-h-[92px] flex-col items-start justify-between rounded-xl border px-4 py-3 text-left transition-all", active ? "border-indigo-500 bg-white shadow-md shadow-indigo-500/10 dark:border-indigo-400 dark:bg-slate-900" : "border-slate-200 bg-white/70 hover:border-indigo-300 dark:border-slate-700 dark:bg-slate-950/40")}><span className="text-sm font-semibold text-slate-900 dark:text-white">{t(providerLabels[item.provider])}</span><span className={cn("text-xs", active ? "text-indigo-600 dark:text-indigo-300" : "text-slate-500")}>{t("rechargeRate")} x{item.recharge_rate || "1"}</span>{item.provider === "stripe" ? methods.length > 0 ? <span className="flex flex-wrap gap-1">{methods.map((method) => <span key={method} className="rounded-md bg-indigo-500/10 px-1.5 py-0.5 text-[10px] font-medium text-indigo-700 dark:text-indigo-300">{t(stripeMethodLabels[method] || "paymentStripeCard")}</span>)}</span> : <span className="rounded-md bg-slate-500/10 px-1.5 py-0.5 text-[10px] font-medium text-slate-600 dark:text-slate-300">{t("paymentStripeAutomatic")}</span> : null}</button>; })}</div></div>
          <div className="flex justify-center border-t border-emerald-500/15 pt-5"><Button type="submit" disabled={!canCreate} className="h-11 min-w-52 gap-2 rounded-xl px-8 shadow-md shadow-indigo-500/20"><BadgeDollarSign className="h-4 w-4" />{busy ? t("rechargeCreating") : t("rechargeCreate")}</Button></div>
        </form>}
