        ...
        <CardContent className="space-y-4 p-0">
          ...
          <div className="overflow-x-auto">
            <Table className="min-w-[1900px]">
              <TableHeader><TableRow><TableHead className="whitespace-nowrap">{t("usageRecordsAPIKey")}</TableHead><TableHead className="whitespace-nowrap">{t("usageRecordsModel")}</TableHead><TableHead className="whitespace-nowrap">{t("usageRecordsReasoning")}</TableHead><TableHead className="whitespace-nowrap">{t("usageRecordsEndpoint")}</TableHead><TableHead className="whitespace-nowrap">{t("usageRecordsIP")}</TableHead><TableHead className="whitespace-nowrap">{t("usageRecordsGroup")}</TableHead><TableHead className="whitespace-nowrap">{t("usageRecordsType")}</TableHead><TableHead className="whitespace-nowrap">{t("usageRecordsBillingMode")}</TableHead><TableHead className="whitespace-nowrap">{t("usageRecordsTokens")}</TableHead><TableHead className="whitespace-nowrap">{t("usageRecordsCost")}</TableHead><TableHead className="whitespace-nowrap">{t("usageRecordsLatency")}</TableHead><TableHead className="whitespace-nowrap">{t("usageRecordsTime")}</TableHead></TableRow></TableHeader>
              <TableBody>
                {busy && !report ? <TableRow><TableCell colSpan={12} className="py-16 text-center text-sm text-slate-500"><RefreshCw className="mx-auto mb-2 h-5 w-5 animate-spin text-indigo-600" />{t("usageRecordsLoading")}</TableCell></TableRow> : !report || report.records.length === 0 ? <TableRow><TableCell colSpan={12} className="py-16 text-center text-sm text-slate-500 dark:text-slate-400">{t("usageRecordsEmpty")}</TableCell></TableRow> : report.records.map((record) => <TableRow key={record.id}>
                  <TableCell className="whitespace-nowrap"><div className="max-w-[180px] truncate font-semibold text-slate-900 dark:text-white">{record.token_name || t("usageRecordsUnnamedToken")}</div><div className="mt-1 font-mono text-[10px] text-slate-500 dark:text-slate-400">{record.token_prefix || "-"}...</div></TableCell>
                  <TableCell className="whitespace-nowrap"><div className="max-w-[190px] truncate font-semibold text-slate-900 dark:text-white" title={record.model}>{record.model}</div><div className="mt-1 text-[10px] uppercase text-slate-500 dark:text-slate-400">{record.provider}</div></TableCell>
                  <TableCell className="whitespace-nowrap text-xs text-slate-600 dark:text-slate-300">{record.reasoning_effort || "-"}</TableCell>
                  <TableCell className="max-w-[170px] truncate whitespace-nowrap font-mono text-xs text-slate-600 dark:text-slate-300" title={record.endpoint}>{record.endpoint || "-"}</TableCell>
                  <TableCell className="whitespace-nowrap font-mono text-xs text-slate-600 dark:text-slate-300">{record.client_ip || "-"}</TableCell>
                  <TableCell className="whitespace-nowrap"><Badge variant="outline">{record.group_name || record.group_code || t("usageRecordsDefaultGroup")}</Badge></TableCell>
                  <TableCell className="whitespace-nowrap"><Badge variant="secondary">{record.request_type === "sync" ? t("usageRecordsSync") : record.request_type || "-"}</Badge></TableCell>
                  <TableCell className="whitespace-nowrap"><Badge variant={record.billing_type === "free" ? "success" : "cyan"}>{record.billing_type === "free" ? t("usageRecordsFree") : t("usageRecordsPrepaid")}</Badge></TableCell>
                  <TableCell className="whitespace-nowrap"><TokenBreakdown input={record.input_tokens} output={record.output_tokens} cached={record.cached_input_tokens} reasoning={record.reasoning_tokens} t={t} /><MeterBreakdown metrics={record.usage_metrics} language={language} label={t("usageRecordsOtherMetrics")} /></TableCell>
                  <TableCell className="whitespace-nowrap font-mono text-xs font-semibold text-emerald-700 dark:text-emerald-300"><div>{t("usageRecordsCustomerCharge")}: {record.currency} {record.status === "settlement_pending" ? `~${formatDecimal(record.estimated_cost)}` : formatDecimal(record.cost)}</div><div className="mt-1 text-[10px] font-normal text-slate-500 dark:text-slate-400">{t("usageRecordsUpstreamCost")}: {record.status === "settlement_pending" ? `~${formatDecimal(record.estimated_upstream_cost)}` : formatDecimal(record.upstream_cost)}</div>{record.status === "settlement_pending" ? <div className="text-[10px] font-normal text-amber-600 dark:text-amber-300">{t("usageRecordsReservedCost")}</div> : null}<ChargeBreakdown lines={record.charge_breakdown} language={language} t={t} /></TableCell>
...
function ChargeBreakdown({ lines, language, t }: { lines?: UsageRecord["charge_breakdown"]; language: Language; t: (key: TranslationKey) => string }) {
  ...
  return <details className="mt-1 max-w-[190px] text-[10px] font-normal text-slate-500 dark:text-slate-400"><summary className="cursor-pointer list-none truncate">{t("usageRecordsChargeDetails")} · {lines.length} {t("usageRecordsChargeLines")}</summary><div className="mt-1 max-w-full overflow-x-auto space-y-0.5 whitespace-nowrap font-mono">{lines.map((line) => <div key={`${line.component_code}-${line.unit}`}>{metricLabel(line.component_code, language)} {formatUsageQuantity(line.quantity)} × {formatUsageQuantity(line.price_per_unit)} = {formatUsageQuantity(line.amount)}</div>)}</div></details>;
}
