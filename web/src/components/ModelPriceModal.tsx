        ...
        <div className="min-h-0 overflow-y-auto px-5 py-5 sm:px-6">
          <form id="model-price-form" className="space-y-5" onSubmit={onSubmit}>
            ...
            <div className="space-y-3 rounded-xl border border-slate-200 p-4 dark:border-slate-800">
              ...
              {form.components.filter((component) => !["input_tokens", "output_tokens", "cached_input_tokens", "reasoning_tokens"].includes(component.component_code)).length === 0 ? <div className="text-xs text-slate-500 dark:text-slate-400">{language === "zh" ? "暂无额外组件。" : "No additional components."}</div> : form.components.map((component, index) => !["input_tokens", "output_tokens", "cached_input_tokens", "reasoning_tokens"].includes(component.component_code) ? <ComponentInput key={String(component.component_code) + "-" + index} component={component} index={index} busy={busy} setForm={setForm} language={language} /> : null)}
            </div>
