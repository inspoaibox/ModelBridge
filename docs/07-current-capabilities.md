...
公告管理位于管理员“组织与安全 -> 公告管理”。公告支持 Markdown、启用状态、生效时间和结束时间；客户控制台只返回当前生效公告，首次进入时对未读公告弹窗提示，右上角入口展示未读数量。关闭弹窗不会标记已读，客户明确确认后才写入 `announcement_user_states.read_at`。管理员可以查看实际触达用户、已读状态和阅读时间。
