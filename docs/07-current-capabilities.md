...
管理员 Session 不能作为下游 Token 使用，租户 Session 不能访问平台管理接口。控制台 Token 列表以及编辑、暂停、启用、终止、删除操作都按 `tenant_id + created_by` 查询，只能操作本人创建的 Token；管理员列表用于平台运营，能看到全平台脱敏摘要。完整密钥只在创建成功响应中出现一次，并只在当前页面会话中可复制。平台所有者可以在独立角色页面维护平台角色、权限集合和已注册管理员绑定，普通平台角色不能越权修改这些绑定。
...
账单与额度页面提供在线充值。系统支持独立启用的微信支付 Native、支付宝当面付、Stripe Checkout 和 PayPal Orders/Capture。支付配置位于管理员“系统设置 -> 支付设置”，敏感配置加密保存；支付订单由平台生成幂等商户单号，只有官方签名回调或官方 API 验证结果才会写入租户账务。Stripe 的 Webhook 地址为当前域名下的 `/payments/webhooks/stripe`，成功/取消返回地址由 Checkout Session 自动生成并回到充值页。Stripe 支付方式可显式勾选卡、Alipay、WeChat Pay，或交给 Stripe 动态选择；Apple Pay/Google Pay 随卡支付钱包资格展示。不同国家的固定白名单规则目前不在平台配置中。
...
管理员可以为渠道配置独立的上游账户查询系统和凭据，用于手动查看第三方中转平台的余额与倍率。该查询是旁路运营信息：不参与候选渠道筛选、调度排序、健康记录、熔断、客户计费或上游成本估算；未配置、查询失败或第三方没有对应接口时，渠道仍按原有配置正常调用。

分组是渠道集合和计费/资源策略，不是模型本身。一个模型可以由多个分组提供，一个分组也可以拥有多个模型。Token 绑定一个分组后，只能使用该分组实际关联渠道提供的模型。

租户控制台提供 `GET /console/v1/tenants/{tenantID}/model-status`。它只返回管理员创建且启用的模型监控配置：管理员可在监控范围内手动指定主模型，租户端按分组呈现一张主模型健康卡，其他模型作为备用模型展开查看。监控选择“全部模型”时动态聚合该分组当前启用的渠道模型映射，选择“指定模型”时保留管理员选择的模型，即使当前没有可用渠道也显示为 `unavailable`。主模型必须属于当前监控范围；未指定时服务端回退为范围内排序最前的模型。活动分组可见，租户已有令牌绑定的停用分组也可见；未配置或已停用监控的分组不会返回。分组停用显示 `disabled`，没有可用路由显示 `unavailable`，部分路由因渠道或模型映射停用、自动熔断不可用显示 `degraded`，所有路由可用且全部路由有真实请求观测显示 `normal`；尚未完成首次观测显示 `pending`。页面按监控配置显示最近 30、60 或 120 次请求状态，默认 60；同时展示最近延迟和近 7 天健康度。该页面是基于数据库运行状态的近实时视图，不主动产生上游请求，前端默认每 15 秒轮询。

模型状态是可选功能，管理员可在系统设置的“功能开关”中关闭。关闭后租户侧入口、深链和接口均不可用，返回 `MODEL_STATUS_DISABLED`；公开 `GET /public/v1/features` 只返回非敏感的功能标志。管理员运营侧的“模型监控”先按分组创建配置，可选择全部模型或指定一个或多个模型，并从当前范围手动设置主模型，设置被展示的最近请求数（30、60、120，默认 60），并可启用按自定义周期运行的主动探测。管理员通过 `GET /admin/v1/model-status` 查看已启用监控覆盖的分组和模型，按分组显示路由可用数、渠道模型熔断状态、最近响应延迟、近 7 天已结束请求可用率和最近请求状态。页面明确区分“最近主动探测”和“最近真实请求”：主动探测使用单字符、1 Token 的最小请求，不计入客户账单或客户使用记录，但会产生上游调用成本；启用或修改后立即执行，最多并发 4 个分组，同一配置不会重叠执行，完成后按配置间隔并加入少量随机抖动安排下一次。近 7 天可用率只统计 `settled`、`settlement_pending` 和 `failed` 请求，其中 `settlement_pending` 表示上游已接受但用量仍待对账。
