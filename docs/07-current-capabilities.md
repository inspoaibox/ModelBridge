...
租户控制台提供 `GET /console/v1/tenants/{tenantID}/model-status`。它只返回管理员创建且启用的模型监控配置：管理员可在监控范围内手动指定主模型，租户端按分组呈现一张主模型健康卡，其他模型作为备用模型展开查看。监控选择“全部模型”时动态聚合该分组当前启用的渠道模型映射，选择“指定模型”时保留管理员选择的模型，即使当前没有可用渠道也显示为 `unavailable`。主模型必须属于当前监控范围；未指定时服务端回退为范围内排序最前的模型。活动分组可见，租户已有令牌绑定的停用分组也可见；未配置或已停用监控的分组不会返回。分组停用显示 `disabled`，没有可用路由显示 `unavailable`，部分路由因渠道或模型映射停用、自动熔断或主动探测失败显示 `degraded`，所有路由可用且全部路由有真实请求或主动探测观测显示 `normal`；尚未完成观测显示 `pending`。页面按监控配置显示最近 30、60 或 120 次请求状态，默认 60；同时展示最近延迟和近 7 天健康度。主动探测不产生客户账单或请求记录，但会参与当前路由健康判断，前端默认每 15 秒轮询。
