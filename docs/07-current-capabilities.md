...
租户控制台提供 `GET /console/v1/tenants/{tenantID}/model-status`。它只返回管理员创建且启用的模型监控配置：监控选择“全部模型”时动态聚合该分组当前启用的渠道模型映射，选择“指定模型”时保留管理员选择的模型，即使当前没有可用渠道也显示为 `unavailable`。活动分组可见，租户已有令牌绑定的停用分组也可见；未配置或已停用监控的分组不会返回。分组停用显示 `disabled`，没有可用路由显示 `unavailable`，部分路由因渠道或模型映射停用、自动熔断不可用显示 `degraded`，所有路由可用且全部路由有真实请求观测显示 `normal`；尚未完成首次观测显示 `pending`。页面按监控配置显示最近 30、60 或 120 次请求状态，默认 60；同时展示最近延迟和近 7 天健康度。该页面是基于数据库运行状态的近实时视图，不是主动上游探针，前端默认每 15 秒轮询。

模型状态是可选功能，管理员可在系统设置的“功能开关”中关闭。关闭后租户侧入口、深链和接口均不可用，返回 `MODEL_STATUS_DISABLED`；公开 `GET /public/v1/features` 只返回非敏感的功能标志。管理员运营侧的“模型监控”先按分组创建配置，可选择全部模型或指定一个或多个模型，设置被展示的最近请求数（30、60、120，默认 60），并可启用按自定义周期运行的主动探测。管理员通过 `GET /admin/v1/model-status` 查看已启用监控覆盖的分组和模型，按分组显示路由可用数、渠道模型熔断状态、最近响应延迟、近 7 天已结束请求可用率和最近请求状态。近 7 天可用率只统计 `settled`、`settlement_pending` 和 `failed` 请求，其中 `settlement_pending` 表示上游已接受但用量仍待对账。
