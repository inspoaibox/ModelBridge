...
账单与额度页面提供在线充值。系统支持独立启用的微信支付 Native、支付宝当面付、Stripe Checkout 和 PayPal Orders/Capture。支付配置位于管理员“系统设置 -> 支付设置”，敏感配置加密保存；支付订单由平台生成幂等商户单号，只有官方签名回调或官方 API 验证结果才会写入租户账务。微信支付和支付宝当面付只对 CNY 预付账户开放；Stripe 是否显示信用卡、微信或支付宝由 Stripe 账户、地区、币种和 Dashboard Payment Methods 决定。
