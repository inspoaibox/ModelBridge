...
返回管理员已启用监控配置覆盖的分组模型路由摘要，包括 `normal`（正常）、`pending`（待观测）、`degraded`（部分可用）、`unavailable`（不可用）和 `disabled`（分组已停用）。`primary_model` 是管理员在当前监控范围内手动指定的主模型；租户端每个分组默认展示它，其他模型作为备用模型展开查看。监控选择“全部模型”时跟随分组当前启用的渠道模型映射；选择“指定模型”时，即使模型暂时没有可用渠道也会保留。活动分组可供租户选择；租户已有令牌绑定的停用分组也会保留显示，帮助解释令牌为何当前不可调用。路由数按该分组实际绑定的渠道模型映射统计；状态来自渠道启停、模型映射启停、自动熔断、主动探测和真实请求成功/失败记录。主动探测不写入客户请求记录，但会参与当前路由健康判断；接口不会返回 Base URL 或密钥。页面进入后默认每 15 秒刷新一次，并支持手动刷新。
...
管理员视图按已启用监控配置列出对应分组和模型，并显示路由可用数、当前状态、主动探测结果、最近延迟、近 7 天真实请求可用率和按配置数量返回的最近请求状态条。管理员通过模型监控新增或编辑接口传入 `primary_model`，服务端只接受当前监控范围内的模型并保存其内部 `models.id`；为空时自动选择范围内第一个模型。选择全部模型时跟随分组当前启用映射；选择指定模型时，即使当前没有可用渠道也保留模型记录并显示不可用。被动模式读取数据库中的渠道映射、熔断健康字段和请求记录；主动模式在保存成功后立即执行首次探测，之后按配置周期使用单字符、1 Token 的最小请求探测上游，不写客户账单或请求记录，但会产生上游调用成本并参与当前路由健康判断。最多并发 4 个分组，同一配置不会重叠执行，完成后加入少量随机抖动再安排下一次。
...
- `401/403` 表示 Token、权限或网络白名单问题；`402` 表示余额不足。`INSUFFICIENT_BALANCE` 响应会额外返回 `currency`、`available_balance` 和 `required_balance`，其中后者是本次请求按输入估算和默认输出上限预占的最低金额；未显式提交 `max_tokens`/`max_completion_tokens` 时，文本请求默认按 1024 输出 Token 预占。`503` 表示价格、分组、凭据或渠道暂时不可用。
