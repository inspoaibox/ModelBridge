...
- `migrations/` 中的迁移按文件名顺序执行，当前发布目录最高版本为 `053_model_monitor_primary_model.sql`；所有环境都应通过应用启动自动执行，发布时必须让二进制、前端和迁移来自同一提交。渠道的上游成本折扣、客户扣费快照和上游参考价快照由 `042`、`043`、`044` 迁移创建，企业认证和支付订单由 `046`、`047` 创建，充值幂等键租户隔离由 `048` 创建，Token 消费限额、分组计量方式、模型状态历史窗口和监控主模型由 `049` 至 `053` 创建。
